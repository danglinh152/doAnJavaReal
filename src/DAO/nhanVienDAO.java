	...
	public ArrayList<nhanVien> xetDuyetCapBac() {
		...
		try {
			...
			String sql = "SELECT DISTINCT NV.MANV, NV.HOTEN, NV.GIOITINH, NV.CAPBAC, NV.NGSINH, NV.SDT, NV.EMAIL, NV.DIACHI, NV.CCCD\n"
					...
					+ "(NVKN.MAKN = 1 AND NVKN.CAPBAC IN ('B')) AND NV.CAPBAC = 'Fresher'))\n" + "AND NV.MANV IN (\n"
					...
					+ "    (NVKN.MAKN = 1 AND NVKN.CAPBAC IN ('C'))AND NV.CAPBAC = 'Junior')) AND NV.MANV IN (\n"
					+ "SELECT DISTINCT NV.MANV\n" + "FROM NHANVIEN NV INNER JOIN HOPDONG HD ON NV.MANV = HD.MANV \n"
					+ "INNER JOIN NHANVIEN_KYNANG NVKN ON NVKN.MANV = NV.MANV\n"
					...
					+ "    (NVKN.MAKN = 1 AND NVKN.CAPBAC IN ('C'))AND NV.CAPBAC = 'Senior')) AND NV.MANV IN (\n"
					+ "SELECT DISTINCT NV.MANV\n" + "FROM NHANVIEN NV INNER JOIN HOPDONG HD ON NV.MANV = HD.MANV \n"
					+ "INNER JOIN NHANVIEN_KYNANG NVKN ON NVKN.MANV = NV.MANV\n"
...
//			errView.setVisible(true);
	...
	public int updateAdmin(nhanVien t) throws Exception {
		int cnt = 0;
